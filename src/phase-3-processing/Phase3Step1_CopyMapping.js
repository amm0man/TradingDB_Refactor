 ...
 *
 * Phase 3 – Step 1: Copy Mapping → Import
 *
 * Sole responsibility:
 *   Copy the current "Schwab Mapping" sheet into the "Import" sheet
 *   using header-name matching (order-independent).
 *   Also performs early Strategy Type / settlement-row handling that
 *   later Phase 3 steps need.
 *
 * Called by:
 ...
 *
 * Related files:
 *   - Phase3Step2_ValidateClean.js   (consumes the Import sheet)
 *   - Phase3BlockLogic.js            (final consumer via Helper)
 */
// =========================================================================
...
function copyMappingToImportByHeaders() {
  ...
  try {
    ...
    // Get headers
    var mappingHeaders = mappingSheet
    ...
    for (let h of tsHeaderNames) {
      const importIdx = importHeaders.findIndex(
        (header) => header.trim().toLowerCase() === h,
      );
      if (importIdx > -1) {
        const mappingIdx = mappingHeaders.findIndex(
          (header) =>
            typeof header === "string" && header.trim().toLowerCase() === h,
        );
    ...
    if (lastMappingRow < 2) {
      uiAlertSafe("No data found to copy from Schwab Mapping.");
      return; // finally will still flush ctx
    }
    var numRows = lastMappingRow - 1;
    var mappingData = mappingSheet
      .getRange(2, 1, numRows, mappingHeaders.length)
      .getValues();
    ...
    const tickerIndex = importHeaders.findIndex(
      (h) => h.trim().toLowerCase() === "ticker",
    );
    const quantityIndex = importHeaders.findIndex(
      (h) => h.trim().toLowerCase() === "quantity",
    );
    const strikeIndex = importHeaders.findIndex(
      (h) => h.trim().toLowerCase() === "option strike",
    );
    const expIndex = importHeaders.findIndex(
      (h) => h.trim().toLowerCase() === "option expiration",
    );
    const cpIndex = importHeaders.findIndex(
      (h) => h.trim().toLowerCase() === "call/put",
    );
    const totalCostIndex = importHeaders.findIndex(
      (h) => h.trim().toLowerCase() === "total cost",
    );
    const signedQuantityIndex = importHeaders.findIndex(
      (h) => h.trim().toLowerCase() === "signed quantity",
    );
    const strategyTypeIndex = importHeaders.findIndex(
      (h) => h.trim().toLowerCase() === "strategy type",
    );
    const tradeTypeIndex = importHeaders.findIndex(
      (h) => h.trim().toLowerCase() === "trade type",
    );
    const entryPriceIndex = importHeaders.findIndex(
      (h) => h.trim().toLowerCase() === "entry price",
    );
    const actionIdx = importHeaders.findIndex(
      (h) => h.trim().toLowerCase() === "action",
    );
    const corpActionsIndex = importHeaders.findIndex(
      (h) => h.trim().toLowerCase() === "corporate actions",
    );
    ...
    for (var r = 0; r < mappingData.length; r++) {
      ...
      var descStr = (descriptionIdx >= 0 ? mappingData[r][descriptionIdx] : "")
        .toString()
        .trim();
      ...
      let totalCostVal =
        totalCostIndex > -1 ? Number(rowArr[totalCostIndex]) || 0 : 0;
      ...
      if (
        upperDesc.includes("TOS TRADES") &&
        upperDesc.includes("BUY") &&
        upperDesc.includes("STOCK") &&
        upperDesc.includes("TO OPEN")
      ) {
        const epForSkip =
          entryPriceIndex > -1 ? Number(rowArr[entryPriceIndex]) || 0 : 0;
        const qtyForSkip =
          quantityIndex > -1 ? Number(rowArr[quantityIndex]) || 0 : 0;
        const expectedCorrectCost = epForSkip * qtyForSkip;

        const isOptionsMultiplierError =
          expectedCorrectCost > 0 &&
          Math.abs(totalCostVal - expectedCorrectCost * 100) /
            (expectedCorrectCost * 100) <
            0.001;
        ...
        if (isOptionsMultiplierError) {
          ...
          importIssuesAdd(
            ...
            "TOS EXERCISE duplicate skipped — TotalCost (" +
              totalCostVal +
              ") is 100× the correct cost (" +
              expectedCorrectCost +
      ...
      if (
        (action === "RAD" ||
          action === "EXP" ||
          upperDesc.includes("EXERCISE") ||
          upperDesc.includes("ASSIGNMENT") ||
          upperDesc.includes("BOT") ||
          upperDesc.includes("UPON") ||
          upperDesc.includes("SYNTHETIC") ||
          upperDesc.includes("REMOVED DUE TO")) &&
        descStr
      ) {
        ...
        if (existingCorpAction) {
          ...
          if (
            (corpUpper === "SYMBOL CHANGE" || corpUpper === "SPLIT") &&
            corpTicker
          ) {
            ...
            if (
              strategyTypeIndex > -1 &&
              !String(rowArr[strategyTypeIndex] || "").trim()
            ) {
          ...
          if (descriptionColInImport !== -1)
            rowArr[descriptionColInImport] = "";
        ...
        const isBotSoldUpon =
          (upperDesc.includes("BOT") || upperDesc.includes("SOLD")) &&
          upperDesc.includes("UPON");

        const looksLikeOptionRemoval =
          !isBotSoldUpon &&
        ...
        if (!isBotSoldUpon && (looksLikeOptionRemoval || action === "EXP")) {
          ...
          if (!parsed) {
            ...
            if (occMatch) {
              ...
              const qtyNum = Number(qtyStr);
              if (quantityIndex > -1) rowArr[quantityIndex] = qtyNum;
              if (signedQuantityIndex > -1)
                rowArr[signedQuantityIndex] = qtyNum;
              if (strikeIndex > -1) rowArr[strikeIndex] = Number(strikeStr);
        ...
        if (
          ...
          const botSoldUponMatch = descStr.match(
            /(BOT|SOLD)\s+([-\d.]+)\s+([A-Z0-9]+)\s+UPON/i,
          );
          if (botSoldUponMatch) {
            ...
            if (actionIdx > -1)
              rowArr[actionIdx] = isSell ? "SELL TO CLOSE" : "BUY TO OPEN";
            if (tickerIndex > -1) rowArr[tickerIndex] = tkr;
            if (quantityIndex > -1) rowArr[quantityIndex] = qtyNum;
            if (signedQuantityIndex > -1)
              rowArr[signedQuantityIndex] = isSell ? -qtyNum : qtyNum;
            if (entryPriceIndex > -1) rowArr[entryPriceIndex] = ep;
            if (totalCostIndex > -1) rowArr[totalCostIndex] = cost;
            if (strategyTypeIndex > -1)
              rowArr[strategyTypeIndex] = isSell ? "" : "LONG STOCK";
            if (tradeTypeIndex > -1) rowArr[tradeTypeIndex] = "Stock";
        ...
        if (
          ...
          if (entryPriceVal > 0 && qtyVal > 0) {
            ...
            if (signedQuantityIndex > -1)
              rowArr[signedQuantityIndex] = Math.abs(qtyVal);
          }
        }
    ...
    const acctIdxFill = importHeaders.findIndex(
      (h) => h.trim().toLowerCase() === "account",
    );
    const tsIdxFill = importHeaders.findIndex(
      (h) => h.trim().toLowerCase() === "trade time stamp",
    );
    ...
    function makeStrategyKey(acct, ticker, exp) {
      ...
      const expKey =
        exp instanceof Date && !isNaN(exp)
          ? exp.getTime().toString()
          : String(exp).trim();
    ...
    for (let fi = 0; fi < outputData.length; fi++) {
      ...
      const fAction =
        actionIdx !== -1 ? fRow[actionIdx].toString().trim().toUpperCase() : "";
      const fTicker =
        tickerIndex !== -1
          ? fRow[tickerIndex].toString().trim().toUpperCase()
          : "";
      const fStrategy =
        strategyTypeIndex !== -1
          ? fRow[strategyTypeIndex].toString().trim().toUpperCase()
          : "";
      const fAcct =
        acctIdxFill !== -1
          ? fRow[acctIdxFill].toString().trim().toUpperCase()
          : "";
    ...
    for (let si = 0; si < outputData.length; si++) {
      ...
      const sAction =
        actionIdx !== -1 ? sRow[actionIdx].toString().trim().toUpperCase() : "";
      const sTicker =
        tickerIndex !== -1
          ? sRow[tickerIndex].toString().trim().toUpperCase()
          : "";
      const sStrategy =
        strategyTypeIndex !== -1
          ? sRow[strategyTypeIndex].toString().trim()
          : "";
      const sAcct =
        acctIdxFill !== -1
          ? sRow[acctIdxFill].toString().trim().toUpperCase()
          : "";
      ...
      const isStockDeliveryLeg =
        (sAction === "BUY TO OPEN" || sAction === "SELL TO CLOSE") &&
        !sStrike &&
        !sExp &&
        !sCp;
      ...
      if (!fillStrategy) {
        ...
        const candidates = Object.keys(strategyFillMap).filter((k) =>
          k.startsWith(prefix),
        );
    ...
    if (lastImportRow >= 4) {
      importSheet
        .getRange(4, 1, lastImportRow - 3, importHeaders.length)
        .clearContent();
    }

    // Paste output
    importSheet
      .getRange(4, 1, finalNumRows, importHeaders.length)
    ...
    uiAlertSafe(finalNumRows + " rows copied from Schwab Mapping to Import.");
  } catch (e) {
