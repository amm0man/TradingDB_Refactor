...
function tosGetAccountFolderIdPropKey(Account) {
  ...
  const primary =
    acct === "LT" ? tosProps.csvFolderIdLT : tosProps.csvFolderIdDT;
  const legacyTrades =
    acct === "LT" ? tosProps.tradesFolderIdLT : tosProps.tradesFolderIdDT;
  const legacyTop =
    acct === "LT" ? tosProps.topFolderIdLT : tosProps.topFolderIdDT;
...
function tosMaybeDebugAlert(message) {
  const enabled =
    String(getSetting("TOS_IMPORT_DEBUG_ALERTS", "0")).trim() === "1";
  if (!enabled) return;
...
function tosSetCsvFolderIdLT() {
  tosSetFolderId(
    ...
    tosProps.csvFolderIdLT,
  );
}

function tosSetCsvFolderIdDT() {
  tosSetFolderId(
    "TOS CSV Folder ID (DT — TosTop + TosTrades)",
    tosProps.csvFolderIdDT,
  );
}
...
/**
 * Reads one Drive CSV into a 2D row array.
 *
 * Shared by tosTradesParseOneCsvFile and tosTopParseOneCsvFile so both
 * section parsers use the same file-read / BOM / delimiter rules.
 * Folder walks are still separate; the next slice can call this once
 * per file and hand the same grid to both parsers.
 *
 * Behavior (must stay identical to the old inlined copies):
 *   - file.getBlob().getDataAsString()
 *   - strip a leading UTF-8 BOM (it breaks exact header matches)
 *   - Utilities.parseCsv as comma-delimited
 *   - if that yields one column AND the text contains tabs, re-parse as tab
 *
 * Does not catch Drive errors. If getBlob() throws, the caller’s try/catch
 * still logs it the same way as before.
 *
 * @param {GoogleAppsScript.Drive.File} file
 * @returns {Array<Array<*>>} parsed grid (may be empty)
 */
function tosReadCsvFileToRows(file) {
...
function pushTosTradesCombinedToTosTrades() {
  ...
  try {
    ...
    function headerIndex(name) {
      ...
      if (j === -1)
        throw new Error("Missing header in Combined Trades: " + name);
      return j;
    }
...
function pushTosTopCombinedToTosTop() {
  ...
  try {
    ...
    if (!src)
      throw new Error("Missing sheet: " + tosConfig.topCombinedSheetName);
    ...
    const values = src.getDataRange().getValues();
    if (values.length < 2)
      throw new Error("No data found in " + tosConfig.topCombinedSheetName);
    ...
    function headerIndex(nameOrNames) {
      ...
      throw new Error(
        "Missing header in Combined Top. Tried: " + names.join(" | "),
      );
    }
    ...
    for (let r = 1; r < values.length; r++) {
      ...
      const dateIsBlank =
        dateVal === "" || dateVal === null || dateVal === undefined;
      ...
      const descUpper = String(row[idx.desc] ?? "")
        .trim()
        .toUpperCase();
      ...
      if (timeRaw && !timeHHmmss) {
        importIssuesAdd(
          ...
          "TIME",
          timeRaw,
          "TIME could not be normalized to HHmmss",
        );
      }
